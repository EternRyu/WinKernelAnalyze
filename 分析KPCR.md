...
当线程进入0环时，FS:[0]指向KPCR(3环时 FS:[0] --> TEB)
...
struct _KPCR
{
    union
    {
        struct _NT_TIB NtTib;  //保存CPU常用的信息(比如异常处理函数链表、栈大小空间限制)                                             //0x0
        struct
        {
            union _KGDTENTRY64* GdtBase;     // GDT表                       //0x0
            struct _KTSS64* TssBase;  // TSS表                              //0x8
            ULONGLONG UserRsp;                                              //0x10
            struct _KPCR* Self;   // 指向自身                                //0x18
            struct _KPRCB* CurrentPrcb; //指向 + 0x120  _KPRCB 结构体        //0x20
    ...
    struct _KPRCB Prcb;     //该结构体为_KPCR的拓展                                                //0x180
}; 
```
